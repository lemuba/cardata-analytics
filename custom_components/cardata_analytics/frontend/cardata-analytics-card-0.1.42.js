...
class CardataAnalyticsMapCard extends HTMLElement {
  ...
  _addPoiToRoute(poi, kind = "waypoint") {
    ...
    if (kind === "destination") {
      ...
      this._routeMessage = `${point.label} wurde als Ziel gesetzt.`;
    } else {
      ...
      this._routeMessage = `${point.label} wurde als Zwischenziel ${this._routeWaypoints.length} hinzugefügt.`;
    }
    this._savePreferences();
  ...
  _bindMapRouteClick() {
    ...
    this._mapRouteClickHandler = (event) => {
      ...
      this._routeMessage = "Kartenziel gesetzt. Google Maps berechnet beim Öffnen die tatsächliche Straßenroute.";
      this._cancelRouteMapPick();
      this._savePreferences();
      this._syncRouteMapSource();
  ...
  _wireEvents() {
    ...
    this.shadowRoot.getElementById("route-toggle")?.addEventListener("click", () => {
      ...
      const panel = this.shadowRoot.getElementById("route-panel");
      panel?.classList.toggle("hidden");
      if (panel && !panel.classList.contains("hidden")) this._renderRoutePanel();
  ...
  _renderRoutePanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="route-section">
        ...
        </div>` : `<div class="route-empty">Ziel über einen POI mit <strong>Als Ziel</strong> setzen oder direkt auf der Karte auswählen.</div>`}
      </div>
      ...
      <div class="route-note">Start ist immer die aktuelle GPS-Position des gewählten Fahrzeugs. Reichweitenhinweise sind bis auf Weiteres reine Luftlinienvergleiche.</div>`;
    ...
    panel.querySelector("#route-vehicle-select")?.addEventListener("change", (event) => {
      ...
      this._routeMessage = "Startfahrzeug geändert. Die Google-Maps-Route verwendet dessen aktuelle GPS-Position.";
      this._savePreferences();
      this._renderRoutePanel();
    });
    ...
    panel.querySelectorAll("[data-route-up]").forEach((button) => button.addEventListener("click", () => {
      ...
      this._routeMessage = "Zwischenziel-Reihenfolge geändert.";
      this._savePreferences();
      this._syncRouteMapSource();
      this._renderRoutePanel();
    }));
    ...
    panel.querySelectorAll("[data-route-remove]").forEach((button) => button.addEventListener("click", () => {
      ...
      this._routeMessage = "Zwischenziel entfernt.";
      this._savePreferences();
      this._syncRouteMapSource();
      this._renderRoutePanel();
      this._updateControls();
    ...
    panel.querySelector("#route-clear-destination")?.addEventListener("click", () => {
      this._routeDestination = null;
      this._routeMessage = "Ziel entfernt.";
      this._cancelRouteMapPick();
      this._savePreferences();
      this._syncRouteMapSource();
      this._renderRoutePanel();
    ...
    panel.querySelector("#route-clear")?.addEventListener("click", () => {
      this._routeWaypoints = [];
      this._routeDestination = null;
      this._routeMessage = "Route gelöscht.";
      this._cancelRouteMapPick();
      this._savePreferences();
      this._syncRouteMapSource();
      this._renderRoutePanel();
  ...
  _styles() {
    return `
      ...
      @container (max-width: 560px) {
        ...
        .route-field select, .route-actions button, .route-actions a, .route-start-card button { min-height:44px; }
