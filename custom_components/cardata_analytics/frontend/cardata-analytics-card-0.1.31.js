...
class CardataAnalyticsMapCard extends HTMLElement {
  ...
  _poiCacheStorageKey() {
    return `${this._storageKey}:poi-cache-v7`;
  }
  ...
  _poiCacheKey(vehicle) {
    ...
    return [
      ...
      Number(this._poiMinPowerKw) || 0, this._poiIncludeUnknownPower ? 1 : 0,
    ].join("|");
  }
  ...
  _poiRequestSnapshot(vehicle, force = false) {
    return {
      ...
      searchFilter: this._poiSearchText.trim(),
  ...
  async _loadPois(force = false, scheduledKey = "") {
    ...
    try {
      ...
      if (stillCurrent) {
        ...
        if (payload.chargingStatus === "building" && request.categories.includes("charging")) {
          retryDelay = Math.max(5000, (Number(payload.retryAfterSeconds) || 10) * 1000);
          this._poiError = "Ladesäulen-Datenbank wird einmalig im Hintergrund aufgebaut … danach laufen Ladefilter und Radius-Suchen lokal ohne Provider-Timeout.";
        }
      }
    } catch (err) {
      const stillOwned = this._poiActiveRequestId === requestId && this._poiLifecycleEpoch === request.epoch;
  ...
  _renderPoiPanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="poi-note">Allgemeine POIs © OpenStreetMap-Mitwirkende · Ladestationen: Bundesnetzagentur.de (CC BY 4.0), lokal gecacht · kein Overpass-Fallback für Ladepunkte · Filter/Vorlagen lokal gespeichert · Cardata ${CARD_VERSION}.</div>`;
