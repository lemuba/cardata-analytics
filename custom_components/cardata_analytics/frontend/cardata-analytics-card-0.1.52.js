...
class CardataAnalyticsMapCard extends HTMLElement {
  ...
  _poiRequestSnapshot(center, force = false) {
    ...
    return {
      ...
      radiusKm: this._poiRadiusKm,
      categories,
      // Typing in the general search stays purely local. Only an explicit
      // Refresh performs a targeted Overpass text query, which can recover
      // matches that fell outside the broad candidate pool.
