...
class CardataAnalyticsCard extends HTMLElement {
  ...
  _repairDialog() {
    ...
    const rows = affected.map((row) => {
      ...
      const reduction = Number(row.reduction_kwh || 0).toLocaleString(this._locale(), { maximumFractionDigits: 2 });
      return `<tr><td>${this._esc(new Date(`${row.date}T12:00:00`).toLocaleDateString(this._locale()))}</td><td>${this._esc(row.spike_count)}</td><td>${this._esc(current)}</td><td>${this._esc(proposed)}</td><td>−${this._esc(reduction)} kWh</td></tr>`;
    }).join("");
