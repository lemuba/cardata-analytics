...
const CARDATA_EN = Object.freeze({
  ...
  "Satellit": "Satellite",
  "OSM Standard (detailliert)": "OSM Standard (detailed)",
...
class CardataAnalyticsMapCard extends HTMLElement {
  ...
  _restorePreferences() {
    try {
      ...
      if (["osm", "osm_detail", "topo", "satellite", "gps"].includes(data.mode)) this._mode = data.mode;
      if (["osm", "osm_detail", "topo", "satellite"].includes(data.lastFreeMode)) this._lastFreeMode = data.lastFreeMode;
        ...
        <div class="map-card-shell">
          ...
          <div class="toolbar">
            <div class="mode-group" role="group" aria-label="${this._esc(this._t("Kartendarstellung"))}">
              ...
              <button class="mode-btn" data-mode="osm_detail" title="${this._esc(this._t("OSM Standard (detailliert)"))}">OSM+</button>
  ...
  _setMode(mode) {
    if (!["osm", "osm_detail", "topo", "satellite", "gps"].includes(mode)) return;
    if (mode === "gps") {
  ...
  _tileProvider() {
    ...
    if (effectiveMode === "osm_detail") {
      return {
        id: "osm-standard-detail",
        maxZoom: 19,
        attribution: `${this._t("Daten")} © <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener">OpenStreetMap contributors</a>`,
        maplibreStyle: this._rasterMapStyle(
          "osm-standard-detail",
    ...
    this._vectorMapInitPromise = ensureCardataMapLibre()
      .then((maplibregl) => {
        ...
        this._vectorMap = new maplibregl.Map({
          ...
          // The official OpenStreetMap raster tile service requires normal
          // browser identification, including an origin Referer. Home Assistant
          // deployments can otherwise inherit a restrictive Referrer-Policy and
          // receive OSM's explicit 403 "Access blocked" tiles. Scope the override
          // strictly to tile.openstreetmap.org so OpenFreeMap, OpenTopoMap, Esri
          // and all other MapLibre requests keep their default behaviour.
          transformRequest: (url) => (
            typeof url === "string" && url.startsWith("https://tile.openstreetmap.org/")
              ? { url, referrerPolicy: "origin" }
              : { url }
          ),
  ...
  _styles() {
    return `
      ...
      @container (max-width: 380px) {
        .mode-group { display:grid; grid-template-columns:repeat(5,minmax(0,1fr)); width:100%; }
