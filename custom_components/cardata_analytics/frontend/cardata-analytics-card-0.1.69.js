...
class CardataAnalyticsMapCard extends HTMLElement {
  ...
  _vehicleGroups() {
    ...
    return [...groups.values()]
      .filter((v) => (v.entities.latitude && v.entities.longitude) || this._trackingStatus?.vehicles?.some(t => t.entry_id === v.entryId && t.session?.last_fix))
  ...
  _vehicleData(v) {
    const e = v.entities;
    const external = this._trackingStatus?.vehicles?.find(t => t.entry_id === v.entryId)?.session?.last_fix;
    const lat = external?.lat ?? this._number(e.latitude);
    const lon = external?.lon ?? this._number(e.longitude);
    ...
    return {
      ...
      address: external ? null : address,
      googleMapsUrl: external ? `https://www.google.com/maps/search/?api=1&query=${lat},${lon}` : googleMapsUrl,
      lastChanged: external?.ts || lastChanged,
    };
  }
  ...
  async _refreshTracking() {
    const panel = this.shadowRoot?.getElementById("tracking-panel");
    if (!this.isConnected || document.hidden || this._gpsDraft || this._gpsBusy || this._trackingRefreshing || this._trackingLoading || this._trackingPlaybackRunning || this._trackingPanelEditing()) return;
    const panelOpen = panel && !panel.classList.contains("hidden");
    if (!panelOpen && this._trackingStatus && !this._trackingStatus.sources?.length) return;
    this._trackingRefreshing = true;
    try {
      await this._loadTrackingStatus(true, true);
      if (panelOpen && this._trackingFollowNow && !this._trackingSelectedTrip && !this._trackingPlaybackEngaged && !this._trackingPanelEditing()) {
  ...
  async _loadTrackingStatus(force = false, quiet = false) {
    ...
    try {
      ...
      this._savePreferences();
      if (this._vectorMap && this._trackingStatus.sources?.length) { this._syncVehicleMapMarkers(); this._renderVehiclePanel(); }
    } catch (err) {
  ...
  _gpsText(de, en) { return cardataLanguage(this._hass) === "de" ? de : en; }

  _gpsSourcesHtml() {
    const esc = (s) => this._esc(s);
    const text = (de, en) => esc(this._gpsText(de, en));
    const sources = this._trackingStatus?.sources || [];
    const vehicles = this._trackingStatus?.vehicles || [];
    const draft = this._gpsDraft;
    return `<div class="tracking-message"><strong>${text("Externe GPS-Quellen", "External GPS sources")}</strong>
      ${vehicles.map(v => {
        const allowed = sources.filter(s => s.vehicles.includes(v.entry_id));
        if (!allowed.length && !v.session?.active) return "";
        const source = sources.find(s => s.id === v.session?.source_id);
        return `<div><b>${esc(v.name)}</b> · ${v.session?.active ? text("Fahrt aktiv", "Trip active") : text("Fahrt beendet / bereit", "Trip ended / ready")}
          ${v.session?.active ? `<div>${esc(source?.name || "")} · ${source?.fix ? text("GPS aktuell", "GPS current") : text("Warte auf gültiges GPS (max. 2 Min. / 100 m)", "Waiting for valid GPS (max. 2 min / 100 m)")}</div><button data-gps-stop="${esc(v.entry_id)}">${text("Fahrt beenden", "End trip")}</button>` : `<select aria-label="GPS" data-gps-choice="${esc(v.entry_id)}">${allowed.map(a => `<option value="${esc(a.id)}" ${a.id === v.session?.source_id ? "selected" : ""}>${esc(a.name)}</option>`).join("")}</select><button data-gps-start="${esc(v.entry_id)}">${text("Fahrt starten", "Start trip")}</button>`}
          ${v.session?.last_fix ? `<small>${text("Letzter Fahrzeugpunkt", "Last vehicle point")}: ${esc(this._formatTrackingDateTime(v.session.last_fix.ts))} · GPS ±${esc(v.session.last_fix.accuracy ?? "–")} m</small>` : ""}</div>`;
      }).join("")}
      ${this._hass?.user?.is_admin ? `<button id="gps-manage">${text("GPS-Quellen verwalten", "Manage GPS sources")}</button>` : ""}
      ${this._gpsManage ? `<div>${sources.map(a => `<div>${esc(a.name)} · ${esc(a.entity_id || a.latitude_entity + " / " + a.longitude_entity)} <button data-gps-edit="${esc(a.id)}">${text("Bearbeiten", "Edit")}</button><button data-gps-delete="${esc(a.id)}">${text("Entfernen", "Remove")}</button></div>`).join("")}<button id="gps-add">${text("Quelle hinzufügen", "Add source")}</button></div>` : ""}
      ${draft ? `<div class="tracking-grid">
        <label class="tracking-field wide">${text("Name", "Name")}<input id="gps-name" value="${esc(draft.name || "")}"></label>
        <label class="tracking-field wide">${text("Standortentität (mit latitude/longitude)", "Location entity (with latitude/longitude)")}<input id="gps-entity" list="gps-entities" value="${esc(draft.entity_id || "")}" placeholder="sensor.matthias_standort"></label>
        <datalist id="gps-entities">${Object.entries(this._hass?.states || {}).filter(([id, st]) => /^(sensor|person|device_tracker)\./.test(id) && st.attributes?.latitude != null && st.attributes?.longitude != null).map(([id]) => `<option value="${esc(id)}"></option>`).join("")}</datalist>
        <label class="tracking-field">${text("Alternativ: Breitengrad-Sensor", "Alternatively: latitude sensor")}<input id="gps-lat" value="${esc(draft.latitude_entity || "")}"></label>
        <label class="tracking-field">${text("Längengrad-Sensor", "Longitude sensor")}<input id="gps-lon" value="${esc(draft.longitude_entity || "")}"></label>
        <div class="tracking-field wide">${text("Für diese Fahrzeuge verfügbar", "Available for these vehicles")}${vehicles.map(v => `<label><input type="checkbox" data-gps-vehicle="${esc(v.entry_id)}" ${(draft.vehicles || []).includes(v.entry_id) ? "checked" : ""}> ${esc(v.name)}</label>`).join("")}</div>
        <small id="gps-preview" class="tracking-field wide"></small>
        <button id="gps-save">${text("Speichern", "Save")}</button><button id="gps-cancel">${text("Abbrechen", "Cancel")}</button>
      </div>` : ""}</div>`;
  }

  async _gpsAction(action, data) {
    if (this._gpsBusy) return;
    this._gpsBusy = true;
    try {
      const result = await this._hass.callWS({type: "cardata_analytics/tracking/sources", action, data});
      this._gpsDraft = null;
      this._trackingMessage = result.waiting ? this._gpsText("Fahrt gestartet. Warte auf aktuellen GPS-Punkt.", "Trip started. Waiting for a current GPS fix.") : this._gpsText("GPS-Einstellung gespeichert.", "GPS setting saved.");
      await this._loadTrackingStatus(true);
      if (this._vectorMap) { this._syncVehicleMapMarkers(); this._renderVehiclePanel(); }
    } catch (err) {
      this._trackingMessage = String(err.message || err);
      this._renderTrackingPanel();
    } finally { this._gpsBusy = false; }
  }

  _bindGpsSources(panel) {
    panel.querySelector("#gps-manage")?.addEventListener("click", () => { this._gpsManage = !this._gpsManage; this._gpsDraft = null; this._renderTrackingPanel(); });
    panel.querySelector("#gps-add")?.addEventListener("click", () => { this._gpsDraft = {vehicles: []}; this._renderTrackingPanel(); });
    panel.querySelector("#gps-cancel")?.addEventListener("click", () => { this._gpsDraft = null; this._renderTrackingPanel(); });
    panel.querySelectorAll("[data-gps-edit]").forEach(el => el.addEventListener("click", () => { this._gpsDraft = {...this._trackingStatus.sources.find(s => s.id === el.dataset.gpsEdit)}; this._renderTrackingPanel(); }));
    panel.querySelectorAll("[data-gps-delete]").forEach(el => el.addEventListener("click", () => {
      if (confirm(this._gpsText("Quelle entfernen? Vorhandene Tracks bleiben erhalten.", "Remove source? Existing tracks will be retained."))) this._gpsAction("delete", {source_id: el.dataset.gpsDelete});
    }));
    panel.querySelectorAll("[data-gps-start]").forEach(el => el.addEventListener("click", () => this._gpsAction("start", {entry_id: el.dataset.gpsStart, source_id: panel.querySelector(`[data-gps-choice="${CSS.escape(el.dataset.gpsStart)}"]`)?.value})));
    panel.querySelectorAll("[data-gps-stop]").forEach(el => el.addEventListener("click", () => this._gpsAction("stop", {entry_id: el.dataset.gpsStop})));
    const capture = () => {
      if (!this._gpsDraft) return;
      Object.assign(this._gpsDraft, {name: panel.querySelector("#gps-name")?.value || "", entity_id: panel.querySelector("#gps-entity")?.value.trim() || "", latitude_entity: panel.querySelector("#gps-lat")?.value.trim() || "", longitude_entity: panel.querySelector("#gps-lon")?.value.trim() || "", vehicles: [...panel.querySelectorAll("[data-gps-vehicle]")].filter(el => el.checked).map(el => el.dataset.gpsVehicle)});
      const d = this._gpsDraft, state = this._hass?.states?.[d.entity_id];
      const lat = d.entity_id ? state?.attributes?.latitude : this._number(d.latitude_entity);
      const lon = d.entity_id ? state?.attributes?.longitude : this._number(d.longitude_entity);
      const preview = panel.querySelector("#gps-preview");
      if (preview) preview.textContent = `${lat ?? "–"}, ${lon ?? "–"} · GPS ±${state?.attributes?.gps_accuracy ?? "–"} m`;
    };
    ["gps-name", "gps-entity", "gps-lat", "gps-lon"].forEach(id => panel.querySelector(`#${id}`)?.addEventListener("input", capture));
    panel.querySelectorAll("[data-gps-vehicle]").forEach(el => el.addEventListener("change", capture));
    panel.querySelector("#gps-save")?.addEventListener("click", () => { capture(); this._gpsAction("save", {...this._gpsDraft, source_id: this._gpsDraft.id}); });
    if (this._gpsDraft) capture();
  }
