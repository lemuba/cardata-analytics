...
// OpenFreeMap provides a public, API-key-free vector basemap. MapLibre is
// loaded lazily only for the normal OSM street-map mode; Topo/Satellite keep
// using the lightweight raster engine below. If another custom card already
// loaded MapLibre, reuse that global instance.
...
class CardataAnalyticsMapCard extends HTMLElement {
  constructor() {
    ...
    // Only one backend POI request may be in flight from this card at a time.
    // Filter/radius changes that happen while a request is running are coalesced
    // into exactly one follow-up request, preventing stale requests from queuing
    // behind Home Assistant's serialized Overpass lock.
    this._poiRequestInFlight = false;
    this._poiReloadPending = false;
    this._poiReloadPendingForce = false;
  ...
  _poiFilterHaystack(poi) {
    return [poi.name, poi.address, poi.operator, poi.brand, poi.network]
      .filter(Boolean).join(" ").toLocaleLowerCase("de-DE");
  }

  _applyPoiClientFilters() {
    const search = this._poiSearchText.trim().toLocaleLowerCase("de-DE");
    const operator = this._poiOperatorText.trim().toLocaleLowerCase("de-DE");
    ...
    this._poiResults = (this._poiRawResults || []).filter((poi) => {
      ...
      if (operator) {
        const operatorHaystack = [poi.operator, poi.brand, poi.network, poi.name]
          .filter(Boolean).join(" ").toLocaleLowerCase("de-DE");
  ...
  _writePoiTemplates(templates) {
    try { localStorage.setItem(this._poiTemplateStorageKey(), JSON.stringify(templates || {})); } catch (_) { /* ignore */ }
  }
  ...
  _poiStatusText() {
    ...
    if (this._poiLoading) return `POIs werden über Home Assistant geladen … · ${this._poiRadiusKm} km`;
    ...
    return `${count} Treffer${filtered ? ` von ${rawCount}` : ""}${this._poiLastEndpoint ? ` · ${this._overpassEndpointLabel(this._poiLastEndpoint)}` : ""}`;
  }
  ...
  _poiCacheStorageKey() {
    return `${this._storageKey}:poi-cache-v6`;
  }
  ...
  _poiCacheKey(vehicle) {
    ...
    return [
      ...
      this._poiSearchText.trim().toLocaleLowerCase("de-DE"),
      this._poiOperatorText.trim().toLocaleLowerCase("de-DE"), this._poiConnector,
    ].join("|");
  }
  ...
  async _fetchOverpass(vehicle, query, force = false) {
    ...
    try {
      const result = await this._hass.callWS({
        ...
        categories: [...this._poiCategories],
        search_filter: this._poiSearchText.trim(),
        operator_filter: this._poiOperatorText.trim(),
        connector_filter: this._poiConnector,
  ...
  async _loadPois(force = false) {
    ...
    // Never queue multiple Home Assistant -> Overpass calls. A filter change while
    // one request is running just remembers that one fresh follow-up is needed.
    if (this._poiRequestInFlight) {
      this._poiReloadPending = true;
      this._poiReloadPendingForce = this._poiReloadPendingForce || Boolean(force);
      this._poiLoading = true;
    ...
    try {
      ...
      const payload = await this._fetchOverpass(vehicle, query, force);
      if (token === this._poiRequestToken) {
        const seen = new Set();
        const results = [];
        ...
        this._poiRawResults = results.slice(0, this._poiMaxResults);
        this._applyPoiClientFilters();
        this._poiSourceVehicleId = vehicle.deviceId;
        this._poiSourceLat = vehicle.lat;
        this._poiSourceLon = vehicle.lon;
        ...
        if (this._poiRawResults.length > 0) {
          cache[cacheKey] = { timestamp: Date.now(), results: this._poiRawResults };
        } else {
      ...
      this._poiRequestInFlight = false;
      const rerun = this._poiReloadPending;
      const rerunForce = this._poiReloadPendingForce;
      this._poiReloadPending = false;
      this._poiReloadPendingForce = false;
      if (rerun) {
        // Keep the spinner active between the stale response and the one newest
        // request; do not let every intermediate UI edit become a network call.
        this._poiLoading = true;
        this._schedulePoiLoad(80, rerunForce);
      } else {
        this._poiLoading = false;
      }
      if (applied || !rerun) {
  ...
  _tileProvider() {
    ...
    // Do not use the donation-funded tile.openstreetmap.org service as an
    // application basemap. OpenFreeMap serves API-key-free vector tiles built
    // from OpenStreetMap data and is rendered locally with MapLibre GL JS.
    return {
      id: "osm-openfreemap-liberty",
      url: null,
      vectorStyle: CARDATA_OPENFREEMAP_STYLE,
      maxZoom: 20,
  ...
  _syncVectorBasemap(provider) {
    ...
    if (!container || !provider?.vectorStyle || this._vectorMapError) return;
    ...
    if (this._vectorMap) {
      try {
        ...
        console.warn("[Cardata Analytics] OpenFreeMap sync failed", err);
      }
      return;
    }
    ...
    this._vectorMapInitPromise = ensureCardataMapLibre()
      .then((maplibregl) => {
        ...
        this._vectorMap.on("error", (event) => {
          const message = event?.error?.message || "OpenFreeMap konnte nicht geladen werden.";
          console.warn("[Cardata Analytics] OpenFreeMap/MapLibre error", event?.error || event);
      ...
      .catch((err) => {
        this._vectorMapError = err?.message || String(err) || "OpenFreeMap konnte nicht geladen werden.";
        console.warn("[Cardata Analytics] OpenFreeMap initialization failed", err);
        this._renderMap(false);
      })
      .finally(() => {
        this._vectorMapInitPromise = null;
  ...
  _renderMap(forceTiles = false) {
    ...
    const useVectorBasemap = Boolean(provider.vectorStyle);
    vectorLayer?.classList.toggle("hidden", !useVectorBasemap);
    tiles.classList.toggle("hidden", useVectorBasemap);
    if (useVectorBasemap) this._syncVectorBasemap(provider);
    ...
    const generationKey = `${provider.id}|${this._zoom}`;
    if (generationKey !== this._tileGenerationKey) {
      tiles.replaceChildren();
      this._tileElements.clear();
      this._tileGenerationKey = generationKey;
    }

    // Incrementally reconcile the visible tile set. Existing <img> nodes stay
    // mounted while the viewport moves, so only newly exposed edge tiles are
    // requested. `forceTiles` still causes an immediate reconciliation through
    // this method but no longer needlessly reloads unchanged tiles.
    const wantedTileKeys = new Set();
    if (provider.url) {
      for (let ty = minY; ty <= maxY; ty++) {
        for (let tx = minX; tx <= maxX; tx++) {
          const tileKey = `${tx}:${ty}`;
          wantedTileKeys.add(tileKey);
          let img = this._tileElements.get(tileKey);
          if (!img) {
            ...
            this._tileElements.set(tileKey, img);
    ...
    for (const [tileKey, img] of [...this._tileElements.entries()]) {
      if (wantedTileKeys.has(tileKey)) continue;
      img.remove();
      this._tileElements.delete(tileKey);
    }
    ...
    if (empty) {
      ...
      } else if (provider.vectorStyle && this._vectorMapError) {
        empty.textContent = `OSM/OpenFreeMap konnte nicht geladen werden: ${this._vectorMapError}`;
        empty.classList.add("show");
  ...
  _renderPoiPanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="poi-filter-section">
        <div class="poi-section-title">Vorlagen</div>
        <div class="poi-template-row">
          <select id="poi-template" aria-label="POI-Vorlage">
            ...
            ${Object.keys(customTemplates).length ? `<optgroup label="Eigene">${Object.entries(customTemplates).sort((a,b) => String(a[1]?.name || a[0]).localeCompare(String(b[1]?.name || b[0]), "de")).map(([key, item]) => `<option value="${this._esc(key)}" ${this._poiActiveTemplate === key ? "selected" : ""}>${this._esc(item.name || key.replace(/^custom:/, ""))}</option>`).join("")}</optgroup>` : ""}
          </select>
          <button id="poi-template-save" title="Aktuelle Filter als eigene Vorlage speichern"><ha-icon icon="mdi:content-save-outline"></ha-icon></button>
          <button id="poi-template-delete" title="Eigene Vorlage löschen" ${this._poiActiveTemplate?.startsWith("custom:") ? "" : "disabled"}><ha-icon icon="mdi:delete-outline"></ha-icon></button>
        </div>
      </div>

      <div class="poi-filter-section">
        <div class="poi-section-title">Kategorien</div>
      ...
      <div class="poi-filter-section poi-filter-grid">
        <label class="poi-field"><span>Suche</span><input id="poi-search" type="search" value="${this._esc(this._poiSearchText)}" placeholder="Name, Marke, Betreiber …"></label>
        <label class="poi-field"><span>Betreiber / Netzwerk</span><input id="poi-operator" list="poi-operator-list" type="search" value="${this._esc(this._poiOperatorText)}" placeholder="z. B. IONITY"></label>
      ...
      <div class="poi-filter-section ${chargingSelected ? "" : "poi-disabled-section"}">
        <div class="poi-section-title">Ladefilter ${chargingSelected ? "" : "· Ladestationen aktivieren"}</div>
      ...
      <div class="poi-note">POI-Daten © OpenStreetMap-Mitwirkende · Filter/Vorlagen lokal gespeichert · Abfrage über Home Assistant / Overpass API · Cardata ${CARD_VERSION}.</div>`;
    ...
    panel.querySelector("#poi-template-save")?.addEventListener("click", () => {
      const current = this._poiActiveTemplate?.startsWith("custom:") ? customTemplates[this._poiActiveTemplate]?.name : "";
      const name = window.prompt("Name der POI-Vorlage:", current || "");
      ...
      const templates = this._readPoiTemplates();
      templates[key] = this._snapshotPoiFilter(clean);
      this._writePoiTemplates(templates);
      this._poiActiveTemplate = key;
      this._renderPoiPanel();
    });
    panel.querySelector("#poi-template-delete")?.addEventListener("click", () => {
      const key = this._poiActiveTemplate;
      if (!key?.startsWith("custom:")) return;
      const templates = this._readPoiTemplates();
      const name = templates[key]?.name || "diese Vorlage";
      if (!window.confirm(`POI-Vorlage „${name}“ löschen?`)) return;
  ...
  _styles() {
    return `
      ...
      .vector-map, .tiles, .poi-markers, .markers { position:absolute; inset:0; overflow:hidden; pointer-events:none; }
      .vector-map.hidden, .tiles.hidden { display:none; }
      ...
      .poi-filter-section { margin-top:8px; padding-top:8px; border-top:1px solid var(--divider-color); }
      .poi-section-title { margin-bottom:6px; color:var(--secondary-text-color); font-size:10px; font-weight:800; text-transform:uppercase; letter-spacing:.04em; }
      .poi-template-row { display:grid; grid-template-columns:minmax(0,1fr) 36px 36px; gap:5px; }
      .poi-template-row select, .poi-template-row button, .poi-field input, .poi-field select { min-height:35px; box-sizing:border-box; border:1px solid var(--divider-color); border-radius:9px; background:var(--card-background-color); color:var(--primary-text-color); padding:5px 8px; }
      ...
      .poi-template-row ha-icon { --mdc-icon-size:17px; }
      .poi-filter-grid { display:grid; grid-template-columns:1fr 1fr; gap:6px; }
      .poi-field { display:flex; flex-direction:column; gap:4px; min-width:0; color:var(--secondary-text-color); font-size:10px; font-weight:700; }
      .poi-field input, .poi-field select { width:100%; min-width:0; font-size:12px; }
      .poi-check-row { display:flex; align-items:center; gap:7px; margin-top:7px; font-size:11px; cursor:pointer; }
      .poi-check-row input { width:17px; height:17px; margin:0; }
      ...
      @container (max-width: 560px) {
        ...
        .poi-categories, .poi-filter-grid { grid-template-columns:1fr; }
      }
    `;
  }
}
