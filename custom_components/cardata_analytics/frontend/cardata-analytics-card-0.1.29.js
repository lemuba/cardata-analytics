...
class CardataAnalyticsMapCard extends HTMLElement {
  ...
  _poiSourceLabel(element = {}) {
    ...
    const labels = rawSources.map((source) => {
      ...
      if (source === "qlever") return "OpenStreetMap · QLever";
  ...
  _renderPoiPanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="poi-note">Allgemeine POIs © OpenStreetMap-Mitwirkende · Ladestationen zusätzlich über QLever OSM und Bundesnetzagentur.de (CC BY 4.0) · Filter/Vorlagen lokal gespeichert · Cardata ${CARD_VERSION}.</div>`;
