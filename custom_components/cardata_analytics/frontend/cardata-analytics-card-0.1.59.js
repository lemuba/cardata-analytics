  ...
  _resetTerrainControls() {
    ...
    if (map && this._mapStyleReady && this._tileProvider()?.terrain === true) {
      ...
      try {
        ...
        if (typeof map.easeTo === "function") map.easeTo({ pitch: this._terrainPitch, bearing: 0, duration: 260, essential: true });
        else map.jumpTo?.({ pitch: this._terrainPitch, bearing: 0 });
      } catch (_) { /* map may be rebuilding */ }
    }
    this._savePreferences();
    this._updateTerrainControls();
  ...
  _syncTerrainView(provider, { animate = true } = {}) {
    ...
    if (terrainEnabled) {
      try {
        ...
        if (!Number.isFinite(currentPitch) || Math.abs(currentPitch - this._terrainPitch) > 0.1) {
          this._programmaticCameraUntil = Date.now() + (animate ? 700 : 250);
          if (animate && typeof map.easeTo === "function") {
            map.easeTo({ pitch: this._terrainPitch, bearing: 0, duration: 320, essential: true });
          } else {
            map.jumpTo?.({ pitch: this._terrainPitch, bearing: 0 });
          }
        }
      } catch (err) {
