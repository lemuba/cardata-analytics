...
class CardataAnalyticsMapCard extends HTMLElement {
  ...
  _snapshotRouteTemplate(name = "") {
    ...
    return {
      ...
      waypoints: this._routeWaypoints.slice(0, 3).map((point) => this._normalizeRoutePoint(point)).filter(Boolean),
      destination,
    };
  }
  ...
  _applyRouteTemplate(key) {
    ...
    this._routeWaypoints = Array.isArray(template.waypoints)
      ? template.waypoints.map((point) => this._normalizeRoutePoint(point)).filter(Boolean).slice(0, 3)
      : [];
  ...
  _addPoiToRoute(poi, kind = "waypoint") {
    ...
    if (kind === "destination") {
      ...
      if (this._routeWaypoints.length >= 3) {
        this._routeMessage = "Maximal 3 Zwischenziele – damit bleibt die Übergabe auch auf dem iPhone kompatibel.";
        this._openRoutePanel();
        return false;
      }
      this._routeWaypoints.push(point);
      this._routeMarkChanged(`${point.label} wurde als Zwischenziel ${this._routeWaypoints.length} hinzugefügt.`);
    }
    this._savePreferences();
    this._syncRouteMapSource();
  ...
  _routeGoogleMapsUrl(navigate = false) {
    ...
    if (this._routeWaypoints.length) {
      params.set("waypoints", this._routeWaypoints.slice(0, 3).map((point) => `${point.lat},${point.lon}`).join("|"));
    }
  ...
  _routeGeoJson() {
    const features = [];
    this._routeWaypoints.slice(0, 3).forEach((point, index) => {
      const p = this._normalizeRoutePoint(point);
      if (!p) return;
      features.push({
        type: "Feature",
  ...
  _bindMapRouteClick() {
    ...
    this._mapRouteClickHandler = (event) => {
      if (this._routePickMode !== "destination") return;
      try {
      ...
      this._routeDestination = this._normalizeRoutePoint({
      ...
      const destinationKey = this._routePointKey(this._routeDestination);
      this._routeWaypoints = this._routeWaypoints.filter((point) => this._routePointKey(point) !== destinationKey);
      this._routeMarkChanged("Kartenziel gesetzt. Google Maps berechnet beim Öffnen die tatsächliche Straßenroute.");
      this._cancelRouteMapPick();
      this._savePreferences();
      this._syncRouteMapSource();
      this._renderRoutePanel();
  ...
  _restorePreferences() {
    try {
      ...
      if (Array.isArray(data.routeWaypoints)) this._routeWaypoints = data.routeWaypoints.map((item) => this._normalizeRoutePoint(item)).filter(Boolean).slice(0, 3);
  ...
  _poiGeoJson() {
    ...
    // Vehicle and route centers have explicit identities. Hide stale results
    // until the follow-up request finishes. Map-center results intentionally
    // remain visible while the user pans and are refreshed on demand/filter change.
    if (this._poiCenterMode !== "map" && currentCenter && currentCenter.key !== this._poiSourceCenterKey) {
      return { type: "FeatureCollection", features: [] };
    }
    return {
  ...
  _renderRoutePanel() {
    const panel = this.shadowRoot?.getElementById("route-panel");
    if (!panel) return;
    panel.classList.toggle("route-picking", this._routePickMode === "destination");
    if (this._routePickMode === "destination") {
      panel.innerHTML = `
        ...
        <div class="route-pick-compact">
          ...
          <div><strong>Ziel auf Karte wählen</strong><small>Tippe oder klicke auf die gewünschte Position. POIs können weiterhin direkt geöffnet werden.</small></div>
    ...
    panel.innerHTML = `
      ...
      <div class="route-section">
        <div class="route-section-head"><span>Zwischenziele</span><strong>${this._routeWaypoints.length}/3</strong></div>
        <div class="route-stop-list">
          ...
          }).join("") : `<div class="route-empty">POI öffnen und <strong>Zwischenziel</strong> wählen. Maximal 3 – auch für iPhone-kompatible Google-Maps-Übergabe.</div>`}
        </div>
      </div>
      ...
      <div class="route-actions">
        ...
        <button id="route-pick-destination" class="${this._routePickMode === "destination" ? "active" : ""}"><ha-icon icon="mdi:map-marker-plus-outline"></ha-icon>${this._routePickMode === "destination" ? "Auf Karte tippen …" : "Ziel auf Karte"}</button>
  ...
  _showPoiPopup(poiId, reposition = true) {
    ...
    popup.innerHTML = `
      ...
      <div class="popup-actions poi-popup-actions">
        <button id="poi-route-waypoint" ${this._routeWaypoints.length >= 3 ? "disabled" : ""}><ha-icon icon="mdi:map-marker-plus-outline"></ha-icon> Zwischenziel</button>
  ...
  _styles() {
    return `
      ...
      .route-search-result { display:grid; grid-template-columns:minmax(0,1fr) 32px 32px; gap:4px; align-items:center; padding:6px; border-radius:8px; background:var(--secondary-background-color); }
      ...
      @container (max-width: 560px) {
        ...
        .route-search-result { grid-template-columns:minmax(0,1fr) 40px 40px; }
