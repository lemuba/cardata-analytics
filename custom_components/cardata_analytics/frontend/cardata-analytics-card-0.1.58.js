...
const CARDATA_TERRAIN_PITCH = 50;
const CARDATA_TERRAIN_EXAGGERATION = 1.5;
      ...
      try {
        if (!map.getSource(CARDATA_TERRAIN_SOURCE_ID)) {
          map.addSource(CARDATA_TERRAIN_SOURCE_ID, {
            type: "raster-dem",
            tiles: [CARDATA_TERRARIUM_TILE_TEMPLATE],
            encoding: "terrarium",
            tileSize: 256,
            minzoom: 0,
            // The public DEM pyramid is deliberately capped for a responsive
            // live vehicle map. MapLibre overzooms this mesh at street zooms.
            maxzoom: 12,
          });
        }
        map.setTerrain({ source: CARDATA_TERRAIN_SOURCE_ID, exaggeration: CARDATA_TERRAIN_EXAGGERATION });
        const currentPitch = Number(map.getPitch?.());
        if (!Number.isFinite(currentPitch) || Math.abs(currentPitch - CARDATA_TERRAIN_PITCH) > 0.1) {
          this._programmaticCameraUntil = Date.now() + (animate ? 700 : 250);
          if (animate && typeof map.easeTo === "function") {
            map.easeTo({ pitch: CARDATA_TERRAIN_PITCH, bearing: 0, duration: 320, essential: true });
          } else {
            map.jumpTo?.({ pitch: CARDATA_TERRAIN_PITCH, bearing: 0 });
          }
        }
      } catch (err) {
