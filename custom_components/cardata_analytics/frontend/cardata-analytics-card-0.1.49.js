...
class CardataAnalyticsMapCard extends HTMLElement {
        ...
        properties: {
          ...
          label: (() => {
            ...
            return motion.speedKmh == null
              ...
              : `${Math.round(rangeKm)} km\nGPS Ø ${Math.round(motion.speedKmh)} km/h`;
          })(),
  ...
  _poiSearchCenter() {
    if (this._poiCenterMode === "route") {
      ...
      return {
        ...
        label: `Routenziel · ${destination.label || destination.address || "Ziel"}`,
        lat: destination.lat,
        lon: destination.lon,
        deviceId: null,
      };
  ...
  _poiStatusText() {
    ...
    const centerLabel = this._poiSourceCenterLabel || center?.label || "POI-Zentrum";
    const radiusNote = this._poiGeneralRadiusLimited() ? " · allgemeine POIs max. 200 km" : "";
    ...
    if (this._poiLoading) {
      ...
      return `POIs werden über Home Assistant geladen … · ${this._poiRadiusKm} km · ${centerLabel}${radiusNote}${retry}`;
    }
    ...
    return `${count} Treffer${filtered ? ` von ${rawCount}` : ""} · ${centerLabel}${radiusNote}${this._poiLastEndpoint ? ` · ${this._overpassEndpointLabel(this._poiLastEndpoint)}` : ""}${duration}${fallback}`;
  }
  ...
  _gpsMotionInfo(deviceId) {
    ...
    if (!raw) return { speedKmh: null, bearingDeg: null, bearingLabel: "", stale: false, status: "Vergleich folgt", quality: "initial" };
    ...
    let status = "Vergleich folgt";
    if (stale) status = "veraltet";
    else if (raw.quality === "jump") status = "GPS-Sprung verworfen";
    else if (raw.quality === "gap") status = "neuer GPS-Punkt · Vergleich folgt";
    else if (speedKmh === 0) status = "steht";
    else if (speedKmh != null) status = bearingLabel ? `fährt · ${bearingLabel}` : "fährt";
  ...
  _gpsMotionCompactText(vehicle) {
    ...
    return `GPS Ø ${speed} · ${motion.status} · ${this._formatAge(vehicle?.lastChanged)}`;
  }
  ...
  _poiRouteCenterChanged() {
    ...
    if (this._normalizeRoutePoint(this._routeDestination)) {
      ...
      this._poiLoading = false;
      this._poiError = "Für dieses POI-Zentrum muss zuerst ein Routenziel gesetzt werden.";
    }
    if (this._domBuilt) this._renderPoiPanel();
  }
  ...
  _applyRouteTemplate(key) {
    ...
    const vehicles = this._vehicles().filter((v) => v.valid);
    const desiredVehicle = String(template.vehicleId || "");
    if (desiredVehicle && vehicles.some((vehicle) => vehicle.deviceId === desiredVehicle)) {
      this._routeVehicleId = desiredVehicle;
    }
    ...
    this._routeMessage = `Routenvorlage „${template.name || "Route"}“ geladen. Start bleibt die aktuelle GPS-Position des Fahrzeugs.`;
  ...
  _savedDestinationPoint(key, item) {
    ...
    return this._normalizeRoutePoint({
      ...
      lat: item.lat,
      lon: item.lon,
      label: item.name || "Gespeichertes Ziel",
  ...
  _setRouteDestination(point, message = "Ziel gesetzt.") {
  ...
  _routeWaypointNotice(message = "") {
    ...
    const note = this._routeUsesMobileGoogleLimit()
      ? `Hinweis: Cardata behält alle ${count} Zwischenziele, übergibt auf diesem mobilen Gerät an Google Maps aber nur die ersten ${ROUTE_MOBILE_GOOGLE_WAYPOINTS}.`
      : `Hinweis: Cardata/Google Maps Desktop kann bis zu ${ROUTE_MAX_WAYPOINTS} Zwischenziele übergeben; mobile Browser garantieren nur ${ROUTE_MOBILE_GOOGLE_WAYPOINTS}.`;
    return [message, note].filter(Boolean).join(" ");
  }

  _addRouteWaypoint(point, message = "Zwischenziel hinzugefügt.") {
    const normalized = this._normalizeRoutePoint(point);
    if (!normalized) return false;
    const key = this._routePointKey(normalized);
    if (this._routeDestination && this._routePointKey(this._routeDestination) === key) {
      this._routeMessage = `${normalized.label} ist bereits das Ziel.`;
      this._cancelRouteMapPick();
      this._renderRoutePanel();
      return false;
    }
    if (this._routeWaypoints.some((item) => this._routePointKey(item) === key)) {
      this._routeMessage = `${normalized.label} ist bereits als Zwischenziel enthalten.`;
      this._cancelRouteMapPick();
      this._renderRoutePanel();
      return false;
    }
    if (this._routeWaypoints.length >= ROUTE_MAX_WAYPOINTS) {
      this._routeMessage = `Maximal ${ROUTE_MAX_WAYPOINTS} Zwischenziele in Cardata.`;
      this._cancelRouteMapPick();
      this._renderRoutePanel();
      return false;
    }
    this._routeWaypoints.push(normalized);
    this._routeMarkChanged(this._routeWaypointNotice(message || `${normalized.label} wurde als Zwischenziel ${this._routeWaypoints.length} hinzugefügt.`));
    this._cancelRouteMapPick();
    this._savePreferences();
    this._syncRouteMapSource();
    this._renderRoutePanel();
  ...
  async _searchRouteAddress(query) {
    ...
    try {
      ...
      if (!this._routeGeocodeResults.length) this._routeGeocodeError = "Keine passende Adresse gefunden.";
    } catch (err) {
      this._routeGeocodeError = `Adresssuche fehlgeschlagen: ${err?.message || err}`;
    } finally {
      this._routeGeocodeLoading = false;
      this._renderRoutePanel();
    }
  ...
  _addPoiToRoute(poi, kind = "waypoint") {
    ...
    if (kind === "destination") {
      ...
      this._cancelRouteMapPick();
      this._routeMarkChanged(`${point.label} wurde als Ziel gesetzt.`);
    } else {
      if (this._routeDestination && this._routePointKey(this._routeDestination) === key) {
        this._routeMessage = "Dieser POI ist bereits das Ziel.";
        this._openRoutePanel();
        return false;
      }
      if (this._routeWaypoints.some((item) => this._routePointKey(item) === key)) {
        this._routeMessage = "Dieser POI ist bereits als Zwischenziel enthalten.";
        this._openRoutePanel();
        return false;
      }
      if (this._routeWaypoints.length >= ROUTE_MAX_WAYPOINTS) {
      ...
      this._routeMarkChanged(this._routeWaypointNotice(`${point.label} wurde als Zwischenziel ${this._routeWaypoints.length} hinzugefügt.`));
    }
    this._savePreferences();
    this._syncRouteMapSource();
  ...
  _routeGoogleMapsUrl(navigate = false) {
    ...
    params.set("api", "1");
    params.set("origin", `${vehicle.lat},${vehicle.lon}`);
  ...
  _bindMapRouteClick() {
    ...
    this._mapRouteClickHandler = (event) => {
      ...
      if (pickMode !== "destination" && pickMode !== "waypoint") return;
      try {
      ...
      const point = this._normalizeRoutePoint({
        ...
        lat,
        lon,
        label: `Kartenpunkt ${lat.toFixed(5)}, ${lon.toFixed(5)}`,
        address: `${lat.toFixed(6)}, ${lon.toFixed(6)}`,
        category: "map",
      });
      if (!point) return;
      if (pickMode === "waypoint") {
        this._addRouteWaypoint(point, `Kartenpunkt wurde als Zwischenziel ${this._routeWaypoints.length + 1} hinzugefügt.`);
      } else {
        this._setRouteDestination(point, "Kartenziel gesetzt. Google Maps berechnet beim Öffnen die tatsächliche Straßenroute.");
      }
    };
  ...
  _fitRoutePoints(save = true) {
    const start = this._routeStartVehicle();
    const points = [
  ...
  async _loadPois(force = false, scheduledKey = "") {
    ...
    if (!center) {
      this._poiError = this._poiCenterMode === "route"
        ? "Für dieses POI-Zentrum muss zuerst ein Routenziel gesetzt werden."
        : "Für die POI-Suche ist kein gültiges Zentrum verfügbar.";
      this._poiLoading = false;
      this._renderPoiPanel();
      return;
    }
      ...
      <ha-card>
        <div class="map-card-shell">
          ...
          <div class="toolbar">
            ...
            <div class="tool-group" role="group" aria-label="Kartenfunktionen">
              <button class="tool-btn" id="fit" title="Alle sichtbaren Fahrzeuge einpassen" aria-label="Alle sichtbaren Fahrzeuge einpassen"><ha-icon icon="mdi:fit-to-screen-outline"></ha-icon><span>Alle</span></button>
              <button class="tool-btn" id="ranges-toggle" title="Restreichweiten aller Fahrzeuge ein-/ausblenden" aria-label="Restreichweiten ein- oder ausblenden"><ha-icon icon="mdi:map-marker-radius-outline"></ha-icon><span>Reichweite</span></button>
              <button class="tool-btn" id="fit-ranges" title="Alle eingeblendeten Restreichweiten einpassen" aria-label="Restreichweiten einpassen"><ha-icon icon="mdi:arrow-expand-all"></ha-icon><span>Bereich</span></button>
              <button class="tool-btn" id="vehicles-toggle" title="Fahrzeuge ein- oder ausblenden" aria-label="Fahrzeuge ein- oder ausblenden"><ha-icon icon="mdi:car-multiple"></ha-icon><span>Fahrzeuge</span></button>
              <button class="tool-btn" id="poi-toggle" title="Points of Interest in Fahrzeugnähe" aria-label="Points of Interest"><ha-icon icon="mdi:map-marker-radius"></ha-icon><span>POIs</span></button>
              <button class="tool-btn" id="route-toggle" title="Route mit POI-Zwischenzielen planen" aria-label="Route planen"><ha-icon icon="mdi:map-marker-path"></ha-icon><span>Route</span></button>
            </div>
          </div>
          <div class="map-wrap">
  ...
  _startDrag(ev) {
    ...
    if (!map || ev.target.closest("button, a, .popup, .poi-popup, .vehicle-panel, .poi-panel, .route-panel")) return;
  ...
  _ensureMapDataLayers() {
    ...
    if (!map.getLayer("cardata-route-points")) {
      map.addLayer({
        ...
        paint: {
          "circle-color": ["case", ["==", ["get", "kind"], "destination"], "#c62828", "#3949ab"],
          "circle-radius": 13,
  ...
  _renderVehiclePanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="panel-actions"><button id="show-all">Alle anzeigen</button><button id="hide-all">Alle ausblenden</button><button id="fit-all-ranges" ${this._rangeVehicles.size ? "" : "disabled"}>Alle + Reichweite</button></div>
        ...
        ${vehicles.map((v) => {
          ...
          <div class="vehicle-row ${v.valid ? "" : "invalid"}" data-vehicle-row="${this._esc(v.deviceId)}" style="--vehicle-color:${this._esc(color)}">
            <label class="vehicle-visible-toggle">
              ...
              <span class="vehicle-row-main"><strong>${this._esc(v.name)}</strong><small>${v.valid ? this._esc(v.address || `${v.lat.toFixed(5)}, ${v.lon.toFixed(5)}`) : "Standort nicht verfügbar"}</small>${v.valid ? `<small class="vehicle-motion">${this._esc(this._gpsMotionCompactText(v))}</small>` : ""}</span>
            </label>
            ...
            <label class="vehicle-range-toggle"><input type="checkbox" data-range-check="${this._esc(v.deviceId)}" ${this._rangeVehicles.has(v.deviceId) ? "checked" : ""} ${rangeAvailable ? "" : "disabled"}> <ha-icon icon="mdi:map-marker-radius-outline"></ha-icon>${rangeAvailable ? `Reichweite ${this._formatNumber(v.range, 0)} km` : "Reichweite nicht verfügbar"}</label>
          </div>`;
        }).join("") || `<div class="panel-empty">Keine Fahrzeuge mit GPS-Konfiguration gefunden.</div>`}
      </div>`;
  ...
  _renderRoutePanel() {
    const panel = this.shadowRoot?.getElementById("route-panel");
    if (!panel) return;
    const routePickActive = this._routePickMode === "destination" || this._routePickMode === "waypoint";
    ...
    if (routePickActive) {
      ...
      panel.innerHTML = `
        ...
        <div class="route-pick-compact">
          <ha-icon icon="${pickingWaypoint ? "mdi:map-marker-plus-outline" : "mdi:flag-checkered"}"></ha-icon>
          <div><strong>${pickingWaypoint ? "Zwischenziel auf Karte wählen" : "Ziel auf Karte wählen"}</strong><small>Tippe oder klicke auf die gewünschte Position. POIs können weiterhin direkt geöffnet werden.</small></div>
          <button id="route-pick-cancel" aria-label="Abbrechen"><ha-icon icon="mdi:close"></ha-icon></button>
        </div>`;
      panel.querySelector("#route-pick-cancel")?.addEventListener("click", () => {
        this._cancelRouteMapPick();
        this._routeMessage = "Kartenauswahl abgebrochen.";
        this._renderRoutePanel();
      });
      return;
    }
    ...
    const googleExportWarning = hasExtendedWaypoints
      ? (mobileGoogleLimit
        ? `Diese Route enthält ${this._routeWaypoints.length} Zwischenziele. Cardata behält alle, aber Google Maps erhält auf diesem mobilen Gerät aus Kompatibilitätsgründen nur die ersten ${ROUTE_MOBILE_GOOGLE_WAYPOINTS}.`
        : `Diese Route enthält ${this._routeWaypoints.length} Zwischenziele und wird auf diesem Gerät vollständig an Google Maps übergeben. Hinweis: Mobile Browser garantieren nur ${ROUTE_MOBILE_GOOGLE_WAYPOINTS} Zwischenziele.`)
      : "";
    ...
    const distanceBadge = (point) => {
      ...
      const rangeText = Number.isFinite(routeRange) && routeRange > 0
        ? (distance <= routeRange ? " · innerhalb Luftlinienbereich" : " · außerhalb Luftlinienbereich")
        : "";
      return `${this._formatNumber(distance, 1)} km Luftlinie${rangeText}`;
    };

    panel.innerHTML = `
      <div class="route-mobile-handle" aria-hidden="true"><span></span></div>
      <div class="panel-title"><span>Routenplanung</span><button id="route-panel-close" aria-label="Schließen"><ha-icon icon="mdi:close"></ha-icon></button></div>

      <div class="route-section route-template-section">
        <div class="route-section-head"><span>Routenvorlage ${this._routeDataLoaded ? "· global" : ""}</span></div>
        <div class="route-template-row">
          <select id="route-template" aria-label="Routenvorlage">
            <option value="" ${!this._routeActiveTemplate ? "selected" : ""}>Aktuelle Route</option>
            ${Object.keys(routeTemplates).length ? `<optgroup label="Eigene · global">${Object.entries(routeTemplates).sort((a,b) => String(a[1]?.name || a[0]).localeCompare(String(b[1]?.name || b[0]), "de")).map(([key, item]) => `<option value="${this._esc(key)}" ${this._routeActiveTemplate === key ? "selected" : ""}>${this._esc(item?.name || key)}</option>`).join("")}</optgroup>` : ""}
          </select>
          <button id="route-template-save" title="Aktuelle Route global speichern" ${destination ? "" : "disabled"}><ha-icon icon="mdi:content-save-outline"></ha-icon></button>
          <button id="route-template-delete" title="Globale Routenvorlage löschen" ${this._routeActiveTemplate?.startsWith("route:") ? "" : "disabled"}><ha-icon icon="mdi:delete-outline"></ha-icon></button>
        </div>
      </div>

      <label class="route-field"><span>Startfahrzeug</span><select id="route-vehicle-select">
        ${vehicles.map((vehicle) => `<option value="${this._esc(vehicle.deviceId)}" ${start?.deviceId === vehicle.deviceId ? "selected" : ""} ${vehicle.valid ? "" : "disabled"}>${this._esc(vehicle.name)}${vehicle.valid ? "" : " · kein GPS"}</option>`).join("")}
      </select></label>
      <div class="route-start-card">
        <ha-icon icon="mdi:car-electric"></ha-icon>
        <div><strong>${this._esc(start?.name || "Kein Fahrzeug mit GPS")}</strong><small>${start ? this._esc(start.address || `${start.lat.toFixed(5)}, ${start.lon.toFixed(5)}`) : "Startposition nicht verfügbar"}</small></div>
        ${start ? `<button id="route-focus-start" title="Startfahrzeug zentrieren"><ha-icon icon="mdi:crosshairs-gps"></ha-icon></button>` : ""}
      </div>

      <div class="route-section">
        <div class="route-section-head"><span>Zwischenziele</span><strong>${this._routeWaypoints.length}/${ROUTE_MAX_WAYPOINTS}</strong></div>
          ...
          ${this._routeWaypoints.length ? this._routeWaypoints.map((item, index) => {
            ...
            return `<div class="route-stop">
              ...
              <div class="route-stop-actions">
                <button data-route-up="${index}" title="Nach oben" ${index === 0 ? "disabled" : ""}><ha-icon icon="mdi:arrow-up"></ha-icon></button>
                <button data-route-down="${index}" title="Nach unten" ${index === this._routeWaypoints.length - 1 ? "disabled" : ""}><ha-icon icon="mdi:arrow-down"></ha-icon></button>
                <button data-route-remove="${index}" title="Entfernen"><ha-icon icon="mdi:close"></ha-icon></button>
              </div>
            </div>`;
          }).join("") : `<div class="route-empty">POI, Adresse oder Kartenpunkt als <strong>Zwischenziel</strong> wählen. Cardata verwaltet bis zu ${ROUTE_MAX_WAYPOINTS} Zwischenziele.</div>`}
        </div>
      ...
      <div class="route-section">
        <div class="route-section-head"><span>Ziel</span>${destination ? `<button id="route-clear-destination" class="route-link-button">Entfernen</button>` : ""}</div>
        ${destination ? `<div class="route-stop route-destination">
          <span class="route-badge">Z</span>
        ...
        </div>` : `<div class="route-empty">Ziel über POI, gespeicherten Ort, Adresse oder direkt auf der Karte setzen.</div>`}
      </div>

      <div class="route-section route-destination-library">
        <div class="route-section-head"><span>Gespeicherte Ziele ${this._routeDataLoaded ? "· global" : ""}</span></div>
        <div class="route-destination-row">
          <select id="route-saved-destination" aria-label="Gespeichertes Ziel">
            <option value="">Ziel auswählen …</option>
            ${Object.keys(savedDestinations).length ? `<optgroup label="Cardata · global">${Object.entries(savedDestinations).sort((a,b) => String(a[1]?.name || a[0]).localeCompare(String(b[1]?.name || b[0]), "de")).map(([key, item]) => `<option value="${this._esc(key)}" ${selectedTarget === key ? "selected" : ""}>${this._esc(item?.name || key)}</option>`).join("")}</optgroup>` : ""}
            ${zones.length ? `<optgroup label="Home Assistant Zonen">${zones.map((zone) => `<option value="${this._esc(zone.key)}" ${selectedTarget === zone.key ? "selected" : ""}>${this._esc(zone.name)}</option>`).join("")}</optgroup>` : ""}
          </select>
          <button id="route-destination-use" title="Ausgewähltes Ziel übernehmen" ${selectedTarget ? "" : "disabled"}><ha-icon icon="mdi:flag-checkered"></ha-icon></button>
        </div>
        <div class="route-mini-actions">
          <button id="route-destination-save" ${destination ? "" : "disabled"}><ha-icon icon="mdi:content-save-outline"></ha-icon>Aktuelles Ziel speichern</button>
          <button id="route-destination-rename" ${selectedGlobalDestination ? "" : "disabled"}><ha-icon icon="mdi:rename-outline"></ha-icon>Umbenennen</button>
          <button id="route-destination-delete" ${selectedGlobalDestination ? "" : "disabled"}><ha-icon icon="mdi:delete-outline"></ha-icon>Löschen</button>
        </div>
      </div>

      <div class="route-section route-address-section">
        <div class="route-section-head"><span>Adresse suchen</span></div>
        <div class="route-address-row">
          <input id="route-address-query" type="search" value="${this._esc(this._routeGeocodeQuery)}" placeholder="Adresse, Ort oder Ziel …" autocomplete="street-address">
          <button id="route-address-search" ${this._routeGeocodeLoading ? "disabled" : ""}><ha-icon icon="mdi:magnify"></ha-icon></button>
        </div>
        ${this._routeGeocodeLoading ? `<div class="route-search-status"><ha-icon icon="mdi:loading" class="spin"></ha-icon> Adresse wird gesucht …</div>` : ""}
        ...
        ${this._routeGeocodeResults.length ? `<div class="route-search-results">${this._routeGeocodeResults.map((item, index) => `<div class="route-search-result">
          <div><strong>${this._esc(item.name || item.address || "Ziel")}</strong><small>${this._esc(item.address || `${Number(item.lat).toFixed(5)}, ${Number(item.lon).toFixed(5)}`)}</small></div>
          <button data-route-search-waypoint="${index}" title="Als Zwischenziel" ${this._routeWaypoints.length >= ROUTE_MAX_WAYPOINTS ? "disabled" : ""}><ha-icon icon="mdi:map-marker-plus-outline"></ha-icon></button>
          <button data-route-search-use="${index}" title="Als Ziel"><ha-icon icon="mdi:flag-checkered"></ha-icon></button>
          <button data-route-search-save="${index}" title="Global speichern"><ha-icon icon="mdi:content-save-outline"></ha-icon></button>
        </div>`).join("")}</div>` : ""}
      </div>

      <div class="route-section route-map-pick-section">
        <div class="route-section-head"><span>Punkt auf Karte</span></div>
        <div class="route-actions route-map-pick-actions">
          <button id="route-pick-waypoint" class="${this._routePickMode === "waypoint" ? "active" : ""}" ${this._routeWaypoints.length >= ROUTE_MAX_WAYPOINTS ? "disabled" : ""}><ha-icon icon="mdi:map-marker-plus-outline"></ha-icon>${this._routePickMode === "waypoint" ? "Auf Karte tippen …" : "Zwischenziel"}</button>
          <button id="route-pick-destination" class="${this._routePickMode === "destination" ? "active" : ""}"><ha-icon icon="mdi:flag-checkered"></ha-icon>${this._routePickMode === "destination" ? "Auf Karte tippen …" : "Ziel"}</button>
        </div>
      </div>
      <div class="route-actions">
        <button id="route-open-pois"><ha-icon icon="mdi:map-marker-radius"></ha-icon>POIs wählen</button>
        <button id="route-fit" ${start && (destination || this._routeWaypoints.length) ? "" : "disabled"}><ha-icon icon="mdi:fit-to-screen-outline"></ha-icon>Einpassen</button>
      </div>
      <div class="route-actions route-google-actions">
        ...
        <a id="route-navigate" class="${navigateUrl ? "" : "disabled"}" href="${this._esc(navigateUrl || "#")}" target="_blank" rel="noopener" aria-disabled="${navigateUrl ? "false" : "true"}"><ha-icon icon="mdi:navigation-variant"></ha-icon>Navigation</a>
      </div>
      <div class="route-actions"><button id="route-clear" ${!destination && !this._routeWaypoints.length ? "disabled" : ""}><ha-icon icon="mdi:delete-outline"></ha-icon>Route löschen</button></div>
      <div class="route-status ${this._routePickMode ? "active" : ""}">${this._esc(this._routeMessage || (destination ? "Cardata übergibt Start, Zwischenziele und Ziel. Die echte Straßenroute berechnet Google Maps." : "Cardata zeichnet nur Routenpunkte – keine irreführende Luftlinienroute."))}</div>
      <div class="route-note">Routen und gespeicherte Ziele sind global in Home Assistant gespeichert. Start ist immer die aktuelle GPS-Position des gewählten Fahrzeugs.</div>`;
    ...
    panel.querySelector("#route-template-save")?.addEventListener("click", async () => {
      ...
      const name = window.prompt("Name der globalen Routenvorlage:", existingName || "");
      ...
      try {
        ...
        this._routeActiveTemplate = key;
        this._routeMessage = `Routenvorlage „${clean}“ global gespeichert.`;
        this._savePreferences();
        this._renderRoutePanel();
      } catch (err) {
        window.alert(`Routenvorlage konnte nicht gespeichert werden: ${err?.message || err}`);
      }
    });
    panel.querySelector("#route-template-delete")?.addEventListener("click", async () => {
      ...
      const name = routeTemplates[key]?.name || "diese Routenvorlage";
      if (!window.confirm(`Globale Routenvorlage „${name}“ löschen?`)) return;
      try {
        await this._deleteGlobalRouteTemplate(key);
        this._routeActiveTemplate = "";
        this._routeMessage = "Routenvorlage gelöscht. Die aktuelle Route bleibt bestehen.";
        this._savePreferences();
        this._renderRoutePanel();
      } catch (err) {
        window.alert(`Routenvorlage konnte nicht gelöscht werden: ${err?.message || err}`);
      }
    });
    panel.querySelector("#route-vehicle-select")?.addEventListener("change", (event) => {
      const id = String(event.target.value || "");
      if (!vehicles.some((vehicle) => vehicle.deviceId === id && vehicle.valid)) return;
      this._routeVehicleId = id;
      this._routeMarkChanged("Startfahrzeug geändert. Die Google-Maps-Route verwendet dessen aktuelle GPS-Position.");
      this._savePreferences();
      this._renderRoutePanel();
    });
    panel.querySelector("#route-focus-start")?.addEventListener("click", () => {
      if (start) this._focusVehicle(start.deviceId, { follow: false, showPopup: false, animate: true });
    });
    panel.querySelectorAll("[data-route-up]").forEach((button) => button.addEventListener("click", () => {
      ...
      this._routeMarkChanged("Zwischenziel-Reihenfolge geändert.");
      this._savePreferences();
      this._syncRouteMapSource();
      this._renderRoutePanel();
    }));
    ...
    panel.querySelectorAll("[data-route-remove]").forEach((button) => button.addEventListener("click", () => {
      ...
      this._routeMarkChanged("Zwischenziel entfernt.");
      this._savePreferences();
      this._syncRouteMapSource();
      this._renderRoutePanel();
      this._updateControls();
    ...
    panel.querySelector("#route-clear-destination")?.addEventListener("click", () => {
      this._routeDestination = null;
      this._routeMarkChanged("Ziel entfernt.");
      this._cancelRouteMapPick();
      this._savePreferences();
      this._syncRouteMapSource();
      this._renderRoutePanel();
    ...
    panel.querySelector("#route-destination-use")?.addEventListener("click", () => {
      ...
      if (!key) return;
      let point = null;
      if (key.startsWith("destination:")) point = this._savedDestinationPoint(key, this._routeDestinations[key]);
      else if (key.startsWith("zone:")) {
        const zone = this._haZoneDestinations().find((item) => item.key === key);
        if (zone) point = this._normalizeRoutePoint({ id: key, lat: zone.lat, lon: zone.lon, label: zone.name, address: zone.address, category: "ha_zone" });
      }
      if (point) this._setRouteDestination(point, `${point.label} wurde als Ziel gesetzt.`);
    });
    panel.querySelector("#route-destination-save")?.addEventListener("click", async () => {
      ...
      if (!point) return;
      const name = window.prompt("Name des globalen Ziels:", point.label || "");
      if (!name || !name.trim()) return;
      const clean = name.trim().slice(0, 80);
      ...
      try {
        ...
        this._routeMessage = `Ziel „${clean}“ global gespeichert.`;
        this._renderRoutePanel();
      } catch (err) {
        window.alert(`Ziel konnte nicht gespeichert werden: ${err?.message || err}`);
      }
    });
    panel.querySelector("#route-destination-rename")?.addEventListener("click", async () => {
      ...
      if (!item) return;
      const name = window.prompt("Neuer Name des globalen Ziels:", item.name || "");
      if (!name || !name.trim()) return;
      const clean = name.trim().slice(0, 80);
      try {
        await this._saveGlobalDestination(key, { ...item, name: clean });
        this._routeMessage = `Ziel in „${clean}“ umbenannt.`;
        this._renderRoutePanel();
      } catch (err) {
        window.alert(`Ziel konnte nicht umbenannt werden: ${err?.message || err}`);
      }
    });
    panel.querySelector("#route-destination-delete")?.addEventListener("click", async () => {
      ...
      const item = key?.startsWith("destination:") ? this._routeDestinations[key] : null;
      if (!item) return;
      if (!window.confirm(`Globales Ziel „${item.name || "Ziel"}“ löschen?`)) return;
      try {
        await this._deleteGlobalDestination(key);
        this._routeSelectedDestinationKey = "";
        this._routeMessage = "Gespeichertes Ziel gelöscht. Eine bereits aktive Route bleibt bestehen.";
        this._renderRoutePanel();
      } catch (err) {
        window.alert(`Ziel konnte nicht gelöscht werden: ${err?.message || err}`);
      }
    });
    ...
    panel.querySelectorAll("[data-route-search-waypoint]").forEach((button) => button.addEventListener("click", () => {
      ...
      if (!item) return;
      const point = this._normalizeRoutePoint({ id: `address:${item.lat},${item.lon}`, lat: item.lat, lon: item.lon, label: item.name || item.address || "Adresse", address: item.address || "", category: "address" });
      if (point) this._addRouteWaypoint(point, `${point.label} wurde als Zwischenziel ${this._routeWaypoints.length + 1} hinzugefügt.`);
    }));
    ...
    panel.querySelectorAll("[data-route-search-save]").forEach((button) => button.addEventListener("click", async () => {
      ...
      if (!item) return;
      const proposed = item.name || item.address || "Ziel";
      const name = window.prompt("Name des globalen Ziels:", proposed);
      if (!name || !name.trim()) return;
      const clean = name.trim().slice(0, 80);
      const key = `destination:${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 7)}`;
      try {
    ...
    panel.querySelector("#route-pick-waypoint")?.addEventListener("click", () => {
      ...
      if (this._routePickMode === "waypoint") {
        ...
        this._routeMessage = "Tippe oder klicke jetzt auf die gewünschte Position für das nächste Zwischenziel.";
    ...
    panel.querySelector("#route-pick-destination")?.addEventListener("click", () => {
      if (this._routePickMode === "destination") {
        ...
        this._routeMessage = "Tippe oder klicke jetzt auf die gewünschte Zielposition in der Karte.";
        try { const canvas = this._vectorMap?.getCanvas?.(); if (canvas) canvas.style.cursor = "crosshair"; } catch (_) {}
      }
      this._renderRoutePanel();
    });
    ...
    panel.querySelector("#route-clear")?.addEventListener("click", () => {
      this._routeWaypoints = [];
      this._routeDestination = null;
      this._routeMarkChanged("Route gelöscht.");
      this._cancelRouteMapPick();
      this._savePreferences();
      this._syncRouteMapSource();
      this._renderRoutePanel();
  ...
  _renderPoiPanel() {
    ...
    ])].sort((a, b) => a.localeCompare(b, "de")).slice(0, 60);
    const chargingSelected = this._poiCategories.has("charging");
    ...
    panel.innerHTML = `
      ...
      <div class="poi-sticky-top">
        ...
        <div class="poi-center-row">
          <label class="poi-field"><span>POI-Zentrum</span><select id="poi-center-mode">
            <option value="vehicle" ${this._poiCenterMode === "vehicle" ? "selected" : ""}>Aktuelles Fahrzeug</option>
            <option value="route" ${this._poiCenterMode === "route" ? "selected" : ""} ${routeCenterAvailable ? "" : "disabled"}>Routenziel${routeCenterAvailable ? "" : " · nicht gesetzt"}</option>
            <option value="map" ${this._poiCenterMode === "map" ? "selected" : ""}>Kartenmitte</option>
          </select></label>
          <div class="poi-center-hint">${this._esc(poiCenter?.label || (this._poiCenterMode === "route" ? "Routenziel fehlt" : "Kein Zentrum verfügbar"))}</div>
        </div>
        <div class="poi-vehicle-row">
          <label class="poi-field"><span>Fahrzeug</span><select id="poi-vehicle-select">
      ...
      <div class="poi-sticky-bottom">
        ...
        <div class="poi-filter-section ${chargingSelected ? "" : "poi-disabled-section"}">
          ...
          ${this._poiOperators.length ? `<div class="poi-operator-chips">${this._poiOperators.map((item, index) => {
            ...
            return `<button type="button" data-remove-poi-operator="${index}" title="${this._esc(item)} entfernen" style="--poi-operator-color:${this._esc(color)}"><span>${this._esc(this._poiOperatorCode(item))}</span>${this._esc(item)}<ha-icon icon="mdi:close"></ha-icon></button>`;
          }).join("")}</div>` : `<div class="poi-selected-empty">Alle Betreiber / Netzwerke</div>`}
          <div class="poi-operator-entry">
            <label class="poi-field"><span>Betreiber hinzufügen</span><input id="poi-operator" list="poi-operator-list" type="search" value="${this._esc(this._poiOperatorText)}" placeholder="z. B. IONITY, EnBW, Tesla" ${chargingSelected ? "" : "disabled"}></label>
            <button id="poi-operator-add" type="button" title="Betreiber hinzufügen" ${chargingSelected && this._poiOperators.length < 12 ? "" : "disabled"}><ha-icon icon="mdi:plus"></ha-icon></button>
          </div>
        ...
        <div class="poi-actions">
          <button id="poi-refresh" ${!selected.length || !poiCenter || this._poiLoading ? "disabled" : ""}><ha-icon icon="mdi:refresh"></ha-icon> Aktualisieren</button>
        ...
        <div class="poi-note">Allgemeine POIs © OpenStreetMap-Mitwirkende · Ladestationen: Open Charge Map · OCM bis 1000 km, allgemeine Overpass-POIs bis 200 km · Vorlagen global in Home Assistant gespeichert · Cardata ${CARD_VERSION}.</div>
      </div>`;
  ...
  _showPoiPopup(poiId, reposition = true) {
    ...
    const rangeHint = distance != null && Number.isFinite(vehicleRange) && vehicleRange > 0
      ? (distance <= vehicleRange ? "im Luftlinien-Reichweitenbereich" : "außerhalb Luftlinien-Reichweitenbereich")
      : "";
    const detailParts = [
    ...
    popup.innerHTML = `
      ...
      <div class="poi-popup-meta"><span>${this._esc(def.label)}</span>${distance != null ? `<span>${this._formatNumber(distance, 1)} km Luftlinie</span>` : ""}${rangeHint ? `<span>${this._esc(rangeHint)}</span>` : ""}</div>
      ...
      <div class="popup-actions poi-popup-actions">
        <button id="poi-route-waypoint" ${this._routeWaypoints.length >= ROUTE_MAX_WAYPOINTS ? "disabled" : ""}><ha-icon icon="mdi:map-marker-plus-outline"></ha-icon> Zwischenziel</button>
        <button id="poi-route-destination"><ha-icon icon="mdi:flag-checkered"></ha-icon> Als Ziel</button>
  ...
  _showPopup(vehicleId, reposition = true) {
    ...
    popup.innerHTML = `
      <div class="popup-head"><div><strong><span class="popup-vehicle-dot" aria-hidden="true"></span>${this._esc(v.name)}</strong><div>${this._esc(address)}</div></div><button id="popup-close" aria-label="Schließen"><ha-icon icon="mdi:close"></ha-icon></button></div>
      <div class="popup-grid">
        ...
        <div><span>GPS Ø</span><strong>${this._esc(motionSpeed)}</strong></div>
        <div><span>Bewegung</span><strong>${this._esc(motion.status)}</strong></div>
        <div><span>GPS-Update</span><strong>${this._esc(this._formatAge(v.lastChanged))}</strong></div>
      </div>
      <div class="popup-actions">
        <button id="popup-follow"><ha-icon icon="mdi:crosshairs-gps"></ha-icon> Folgen</button>
        <button id="popup-route"><ha-icon icon="mdi:map-marker-path"></ha-icon> Route</button>
        ${Number.isFinite(Number(v.range)) && Number(v.range) > 0 ? `<button id="popup-range"><ha-icon icon="mdi:map-marker-radius-outline"></ha-icon> ${rangeVisible ? "Ring aus" : "Ring an"}</button>` : ""}
  ...
  _styles() {
    return `
      ...
      .vehicle-panel.hidden, .poi-panel.hidden, .route-panel.hidden, .popup.hidden, .poi-popup.hidden { display:none; }
      ...
      .route-destination-row { grid-template-columns:minmax(0,1fr) 34px; }
      ...
      .route-search-result { display:grid; grid-template-columns:minmax(0,1fr) 32px 32px 32px; gap:4px; align-items:center; padding:6px; border-radius:8px; background:var(--secondary-background-color); }
      ...
      @container (max-width: 820px) {
        ...
        .tool-group { display:grid; grid-template-columns:repeat(6,minmax(36px,1fr)); gap:5px; width:100%; min-width:0; }
        .tool-btn { width:100%; min-width:0; padding:0 4px; }
        .tool-btn span { display:none; }
      }
      @container (max-width: 380px) {
        .mode-group { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); width:100%; }
        .mode-btn { min-width:0; padding:0 3px; gap:2px; font-size:11px; }
      ...
      @container (max-width: 560px) {
        ...
        .route-destination-row { grid-template-columns:minmax(0,1fr) 44px; }
        ...
        .route-search-result { grid-template-columns:minmax(0,1fr) 40px 40px 40px; }
