...
const CARDATA_EN = Object.freeze({
  ...
  "Routen und gespeicherte Ziele sind global in Home Assistant gespeichert. Start ist immer die aktuelle GPS-Position des gewählten Fahrzeugs.": "Routes and saved destinations are stored globally in Home Assistant. The start is always the selected vehicle's current GPS position.",
  ...
  "Routenvorlage „{name}“ geladen. Start bleibt die aktuelle GPS-Position des Fahrzeugs.": "Route template “{name}” loaded. The start remains the vehicle's current GPS position.",
...
class CardataAnalyticsMapCard extends HTMLElement {
  ...
  _poiStatusText() {
    ...
    const resultText = filtered
      ? this._t("{count} Treffer von {raw}", { count, raw: rawCount })
      : this._t("{count} Treffer", { count });
    return `${resultText} · ${centerLabel}${radiusNote}${this._poiLastEndpoint ? ` · ${this._overpassEndpointLabel(this._poiLastEndpoint)}` : ""}${duration}${fallback}`;
  }
  ...
  _applyRouteTemplate(key) {
    ...
    this._routeMessage = this._t("Routenvorlage „{name}“ geladen. Start bleibt die aktuelle GPS-Position des Fahrzeugs.", { name: template.name || this._t("Route") });
    ...
    try {
      ...
      if (stillCurrent) {
        ...
        this._poiError = results.length >= request.maxResults
          ? this._t("Die Anzeige ist auf {max} POIs begrenzt. Radius oder Filter ggf. verkleinern.", { max: request.maxResults })
          : "";
        const cache = this._readPoiCache();
      ...
      if (stillCurrent) {
        ...
        if (transient && this._poiRetryCount <= this._poiMaxAutoRetries) {
          ...
          this._poiError = this._t("POIs konnten nicht geladen werden: {error}", { error: err?.message || err });
        }
      }
    } finally {
  ...
  _renderRoutePanel() {
    ...
    if (routePickActive) {
      ...
      panel.innerHTML = `
        ...
        <div class="route-pick-compact">
          ...
          <div><strong>${this._esc(this._t(pickingWaypoint ? "Zwischenziel auf Karte wählen" : "Ziel auf Karte wählen"))}</strong><small>${this._esc(this._t("Tippe oder klicke auf die gewünschte Position. POIs können weiterhin direkt geöffnet werden."))}</small></div>
    ...
    panel.innerHTML = `
      ...
      <label class="route-field"><span>${this._esc(this._t("Startfahrzeug"))}</span><select id="route-vehicle-select">
        ${vehicles.map((vehicle) => `<option value="${this._esc(vehicle.deviceId)}" ${start?.deviceId === vehicle.deviceId ? "selected" : ""} ${vehicle.valid ? "" : "disabled"}>${this._esc(vehicle.name)}${vehicle.valid ? "" : ` · ${this._t("kein GPS")}`}</option>`).join("")}
      </select></label>
      <div class="route-start-card">
        <ha-icon icon="mdi:car-electric"></ha-icon>
        <div><strong>${this._esc(start?.name || this._t("Kein Fahrzeug mit GPS"))}</strong><small>${start ? this._esc(start.address || `${start.lat.toFixed(5)}, ${start.lon.toFixed(5)}`) : this._t("Startposition nicht verfügbar")}</small></div>
        ${start ? `<button id="route-focus-start" title="${this._esc(this._t("Startfahrzeug zentrieren"))}"><ha-icon icon="mdi:crosshairs-gps"></ha-icon></button>` : ""}
      </div>
      ...
      <div class="route-section route-destination-library">
        <div class="route-section-head"><span>${this._esc(this._t("Gespeicherte Ziele"))} ${this._routeDataLoaded ? "· global" : ""}</span></div>
        <div class="route-destination-row">
          <select id="route-saved-destination" aria-label="${this._esc(this._t("Gespeichertes Ziel"))}">
            <option value="">${this._esc(this._t("Ziel auswählen …"))}</option>
      ...
      <div class="route-note">${this._esc(this._t("Routen und gespeicherte Ziele sind global in Home Assistant gespeichert. Start ist immer die aktuelle GPS-Position des gewählten Fahrzeugs."))}</div>`;
    ...
    panel.querySelector("#route-vehicle-select")?.addEventListener("change", (event) => {
      ...
      this._routeMarkChanged(this._t("Startfahrzeug geändert. Die Google-Maps-Route verwendet dessen aktuelle GPS-Position."));
      this._savePreferences();
      this._renderRoutePanel();
    });
