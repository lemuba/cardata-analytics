...
class CardataAnalyticsMapCard extends HTMLElement {
  ...
  _poiStatusText() {
    ...
    return `${count} Treffer${filtered ? ` von ${rawCount}` : ""}${this._poiLastEndpoint ? ` · ${this._overpassEndpointLabel(this._poiLastEndpoint)}` : ""}${duration}`;
  }
    ...
    try {
      ...
      if (stillCurrent) {
        ...
        if (this._poiRawResults.length > 0) {
          cache[request.key] = { timestamp: Date.now(), results: this._poiRawResults };
        } else {
