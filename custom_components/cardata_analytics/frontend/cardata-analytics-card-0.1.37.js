...
class CardataAnalyticsMapCard extends HTMLElement {
    ...
    this._vectorMapInitPromise = ensureCardataMapLibre()
      .then((maplibregl) => {
        ...
        this._vectorMap.on("movestart", (event) => {
          if (event?.originalEvent && this._mode === "gps") {
    ...
    for (const vehicle of visible) {
      ...
      if (!item) {
        ...
        element.addEventListener("click", (ev) => {
          ...
          if (this._mode === "gps") this._center = { lat: current.lat, lon: current.lon };
          this._savePreferences();
          this.shadowRoot?.getElementById("poi-popup")?.classList.add("hidden");
  ...
  _renderPoiPanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="poi-filter-section">
        ...
        <div class="poi-category-groups">
          ${this._poiCategoryGroups().map((group, groupIndex) => {
            const entries = Object.entries(defs).filter(([, def]) => def.group === group.key);
            return `<details class="poi-category-group" data-poi-group="${this._esc(group.key)}" ${groupIndex < 2 ? "open" : ""}>
              <summary>${this._esc(group.label)} <span>${entries.length}</span></summary>
      ...
      <div class="poi-note">Allgemeine POIs © OpenStreetMap-Mitwirkende · Textsuche lokal auf dem geladenen Kategorie-Cache · Ladestationen: Open Charge Map, serverseitig gecacht · kein Overpass-Fallback für Ladepunkte · Filter/Vorlagen lokal gespeichert · Cardata ${CARD_VERSION}.</div>`;
    ...
    panel.querySelectorAll("[data-poi-category]").forEach((input) => {
      input.addEventListener("change", () => {
        ...
        if (key === "charging" && chargingWasSelected && !input.checked) {
          // Charging-specific text and connector/power filters must never leak
          // into cafés, restaurants or other normal POI categories.
          this._clearChargingSpecificFilters(true);
        }
        this._poiActiveTemplate = "";
        this._savePreferences();
        this._selectedPoiId = null;
  ...
  _renderPoiMarkers(topLeft, width, height, n) {
    ...
    // features rendered and clustered directly by MapLibre in 0.1.37+.
  ...
  _styles() {
    return `
      ...
      .poi-category-search { margin-bottom:7px; }
      .poi-category-groups { display:flex; flex-direction:column; gap:6px; }
      .poi-category-group { border:1px solid var(--divider-color); border-radius:9px; background:color-mix(in srgb, var(--secondary-background-color) 65%, transparent); overflow:hidden; }
      .poi-category-group > summary { display:flex; align-items:center; justify-content:space-between; gap:8px; padding:7px 8px; cursor:pointer; font-size:11px; font-weight:750; user-select:none; }
      .poi-category-group > summary span { color:var(--secondary-text-color); font-size:9px; font-weight:600; }
      .poi-category-group[open] > summary { border-bottom:1px solid var(--divider-color); }
      .poi-category-group-filtered { display:none; }
      .poi-categories { display:grid; grid-template-columns:1fr 1fr; gap:5px; padding:6px; }
