    ...
    async with asyncio.timeout(35.0):
        async with session.get(
            ...
            headers={
                ...
                "User-Agent": "Cardata Analytics/0.1.64 (https://github.com/lemuba/cardata-analytics)",
            },
    ...
    for page_url in BNETZA_PAGE_URLS:
        try:
            async with asyncio.timeout(12.0):
                async with session.get(page_url, headers={"User-Agent": "Cardata Analytics/0.1.64"}) as response:
                    if response.status != 200:
                ...
                async with asyncio.timeout(6 * 60):
                    async with session.get(
                        ...
                        headers={
                            ...
                            "User-Agent": "Cardata Analytics/0.1.64",
                        },
                        allow_redirects=True,
                    ) as response:
                        if response.status != 200:
